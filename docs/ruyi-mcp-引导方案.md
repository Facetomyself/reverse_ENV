...
| ruyipage | 1.2.50（`.venv\Lib\site-packages\ruyipage\`） | 已验证 |
...
- ruyipage 1.2.50 是成熟的 Python 包，直接重写成本高、风险大
...
| `ruyi_set_fingerprint` | `fingerprint.set` | `opts.smart_fingerprint()` / `page.set_window_size()` |
...
| `page.actions.hold/wait/human_move/release()` | 原子拖拽 | 已通过 1.2.50 真浏览器验证 |
| `page.set_window_size()` | 同步 outer/viewport/screen/DPR | 已通过 1.2.50 真浏览器验证 |
