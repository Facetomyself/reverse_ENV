...
| ruyipage | 1.2.54（`.venv\Lib\site-packages\ruyipage\`） | 已验证 |
...
- ruyipage 1.2.54 是成熟的 Python 包，直接重写成本高、风险大
...
早期 43 tools 设计基线中先选出下列 8 个工具验证架构；当前实现已扩展为 57 tools：
...
> 当前已落地 57 tools。MCP 内的 3 个 `ruyi_trace_*` 工具是 BiDi JSON Trace；C++ DOMTrace 始终由独立 `tools\ruyitrace\ruyitrace.ps1` 承担。
...
│ dom_get_info     │ │ requests         │ │ intercept_res    │ │  57 tools 齐全)  │
...
| `ruyi_capture_start` | `network.capture_start` | `page.capture.start(...)` |
| `ruyi_capture_wait` | `network.capture_wait` | `page.capture.wait(...)`；Bridge 固定输出 `packets` 数组 |
| `ruyi_capture_stop` | `network.capture_stop` | `capture.clear()` 后按 `cleanupTimeout` 有界执行 `page.capture.stop()` |
...
| `ruyi_select_frame` | `frame.select` | `page.get_frame(context_id=...)` 或 `page.get_frame(locator=selector)` 的 `iframe.contentWindow` 精确映射 |
| `ruyi_clear_site_data` | `browser.clear_data` | `page.delete_cookies()` + Storage clear |
| **ruyi 独有 ↓** | | |
| `ruyi_set_proxy` | `proxy.set` | `opts.set_proxy(url)` |
| `ruyi_set_fingerprint` | `fingerprint.set` | `opts.smart_fingerprint()` / `page.set_window_size()` / `page.set_viewport()` / `page.emulation.set_screen_size()` |
...
| `ruyi_human_drag` | `human.drag` | `move_to().hold().wait().human_move().release().perform()` |
...
被动抓包的 MCP 调用顺序固定为 `ruyi_capture_start -> ruyi_capture_wait -> ruyi_capture_stop`。需要的 packet 必须在 stop 前取走；v0.1.5 的 stop 会丢弃剩余内部队列/历史，再按 `cleanupTimeout`（默认 5 秒，范围 0.1–30 秒）完成有界清理。
...
| `page.capture.start/wait/stop()` | 网络抓包 | `count=1` 返回单个 `CapturePacket` / `None`，`count>1` 返回 list；MCP Bridge 统一为 `packets` 数组，stop 前先 wait |
...
| `page.actions.hold/wait/human_move/release()` | 原子拖拽 | 已通过 1.2.54 真浏览器验证 |
| `page.set_window_size()` | 仅设置 outer window | 已通过 1.2.54 真浏览器验证 |
| `page.set_viewport()` / `page.emulation.set_screen_size()` | 独立设置 viewport+DPR / `screen.*` | 已通过 1.2.54 真浏览器验证；当前 runtime 可能忽略 screen DPR |
| `page.get_frame(context_id=...)` / `page.get_frame(locator=...)` | 稳定 context 选择 / `iframe.contentWindow` 精确映射 | 已通过同 URL / `srcdoc` 真浏览器验证 |
| `page.new_tab(url=None)` / `page.new_container_tab(url=None)` | 先创建空白 tab，再在首跳前重放 fingerprint | 已验证；container 创建失败不得降级 |
