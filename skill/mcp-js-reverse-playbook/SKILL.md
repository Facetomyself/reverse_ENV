...
> 遵循 `reverse-coordinator` 约定：不得跳过 Observe 直接猜环境。WASM/VM marker 先交 `web-deobfuscation` gate 分级，只有缺 opcode/boundary Trace、fixture 或稳定语义时才标 L4 triage。
...
- 如果要长期复用算法链路，这一阶段必须做
- 已确认 AST 混淆、JSVMP 或 WASM 边界是核心阻塞时，切 `web-deobfuscation`；本 skill 负责提供源码、paused/runtime 证据和请求 fixture，不自行宣称算法恢复
...
- AST / JSVMP / WASM 路由：`references/ast-deobfuscation.md`
