---
name: apple-design
description: Apple 界面设计哲学 → Web — 流体交互、弹簧物理、手势、材质深度、排版。触发：Apple 风格/iOS 感觉/怎么做弹簧动画/手势交互。
parent: emil-design-eng（编排器）
route: 编排器自动路由，"Apple风格/iOS感觉/流体交互/弹簧动画怎么做"触发
---
...
animateSpringTo(target, { velocity: releaseVelocity }); // then hand off velocity (S5)
```

Note: the physics-textbook `v squared/(2 * decel)` is *not* what Apple ships — use the exponential-decay form above. This is the standard behavior in good bottom-sheets and carousels (Vaul, Embla).
...
- **Mirror the easing on reversible transitions** so the outbound path matches the return path (use inverse cubic-bezier control points for the two directions).
