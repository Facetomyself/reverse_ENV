...
第三方/改编资产来源、license 和 hash 占位统一记录在 `third_party_manifest.json`。更新或重分发内置工具前，必须把对应 TODO hash 替换为实际 sha256，并补齐待验证 license。
...
| Frida SO 隐身 / 信号链保护 / VMA 重命名 | `third_party/shadow-hook/stealth-runner.py` | `shadow-hook-tools.md` |
| TLS 明文/pcap/keylog | `ecapture_android.py` | `stealth-ecapture-tools.md` |

`install_skill_tools.py` 会把 Skill 内置 `scripts/tools/shadow-hook/` 整目录复制到工程 `third_party/shadow-hook/`。优先运行工程副本，避免从 Skill 原目录直接写入 gadget 配置或依赖错误的项目根目录。
