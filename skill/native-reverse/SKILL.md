...
- **syscall-filter / stealth-hook**：仅在准备使用前确认 APatch/KernelPatch、KPM 加载能力和匹配 ABI；KernelPatch superkey 优先由显式参数传入，其次使用 `KP_SUPERKEY`，兼容 `XJB_KP_SUPERKEY`。
...
16. **禁止自行更换 Frida 版本**：发现宿主 Frida 与设备端 frida-server 版本不匹配时，只能记录风险并建议用户自行更换；禁止自行安装/升级 Python Frida 包、创建/切换 venv、推送替换 frida-server 或改用其它版本。详见 `references/workflow-standards.md` §11。
...
- 需要 Shadow Hook (隐身工具): 读 `references/shadow-hook-tools.md`。
...
- Frida 无法注入时用 CPU 模拟执行：读 `references/unicorn-emulation.md`（ARM64/x86/MIPS 多架构 Unicorn 模拟 + JNI/syscall hook 策略）。
