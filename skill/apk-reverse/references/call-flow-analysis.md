...
4. **Recover evidence-backed Kotlin names**: run `scripts/recover-kotlin-names.sh` to build a high-confidence map from `@DebugMetadata.c` and jadx rename comments. `@Metadata.d2` descriptors are dependency references and remain low-confidence candidates until independently correlated. Coverage is sample-dependent; see [`kotlin-name-recovery.md`](./kotlin-name-recovery.md).
