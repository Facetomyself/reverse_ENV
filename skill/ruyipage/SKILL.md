...
1. **`smart_fingerprint` 依赖代理出口 IP** — 无代理时无法检测地理位置，指纹伪装不完整
2. **不要与 ruyitrace 共享同一 Firefox profile** — 两者用独立 profile，避免相互污染浏览器状态
3. **代理认证走 fpfile** — 代理用户名密码用 `set_fpfile` 引用外部文件，避免硬编码
