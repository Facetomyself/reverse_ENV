#!/usr/bin/env python3
"""Cliproxy 代理测试工具 — 验证连接、Sticky/Rotating 模式、地区出口。

用法:
    # 环境变量 (推荐)
    $env:CLIPROXY_USER = "你的用户名"
    $env:CLIPROXY_PASS = "你的密码"
    python cliproxy_test.py

    # 命令行
    python cliproxy_test.py --user "xxx" --pass "yyy"

    # 指定地区 + 测试 Sticky
    python cliproxy_test.py --region US --state California --sticky 30

    # 完整测试: Rotating vs Sticky 对比
...
def test_proxy(account: str, password: str, host: str = CLIPROXY_HOST,
    ...
    """通过 SOCKS5 代理请求目标 URL，返回结果。"""
    proxy_url = f"socks5h://{account}:{password}@{host}:{port}"
...
def main():
    parser = argparse.ArgumentParser(description="Cliproxy 代理测试工具")
    ...
    parser.add_argument("--host", default=CLIPROXY_HOST, help=f"SOCKS5 主机 (默认 {CLIPROXY_HOST})")
    parser.add_argument("--port", type=int, default=CLIPROXY_PORT, help=f"SOCKS5 端口 (默认 {CLIPROXY_PORT})")
    ...
    print(f"  Result: {unique_rot}/3 unique IPs" + (" (PASS)" if unique_rot >= 2 else " (可能服务端缓存了)"))
    ...
    print(f"  Result: {unique_sticky}/3 unique IPs" + (" (PASS)" if unique_sticky == 1 else " (IP变了, 检查 sid)" if unique_sticky > 1 else ""))
