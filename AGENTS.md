...
> Claude Bash 命令额外经过 `.claude/settings.json` 注册的 `PreToolUse` hook：`.claude/hooks/pre-tool-policy.ps1`。该 hook 拦截 `reset --hard`、强制清理、强推和受保护根目录递归删除，并对普通 push / 宽泛进程终止给出提醒；它只是一道前置护栏，不能替代 branch/upstream、工作树归属和提交前检查。
...
| `apk-reverse` | Android APK/JAR/AAR | jadx/apktool/frida/adb + 指纹/脱壳/Kotlin类名恢复/API提取/Vineflower 对照 |
...
| dex2jar 2.4.31 | `tools\dex2jar\dex-tools-2.4.31\` |
